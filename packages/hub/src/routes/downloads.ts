...
async function authorizeNodeConfigDownload(
  ...
  { ok: true; tenantId: string; signedClaims: null | { mirrorUpdatedAt: string } }
  | { ok: false; status: number; error: string }
> {
  const token = asString(req.query.token);
  const claims = token ? verifyDownloadToken(token) : null;
...
export function createDownloadsRouter(): Router {
  ...
  router.get('/bundle/windows/latest', async (req: Request, res: Response) => {
    ...
    try {
      ...
      return fs.createReadStream(bundle.path).pipe(res);
    } catch (error) {
      console.error('[downloads] Failed to stream installer bundle', error);
      if (!res.headersSent) {
        return res.status(500).json({ error: 'Failed to prepare installer bundle.' });
      }
      res.end();
    }
  });
  ...
  router.get('/nodes/:nodeId/config.yaml', async (req: Request, res: Response) => {
    ...
    const activeAgentToken = await getActiveAgentToken(nodeId, auth.tenantId);
    if (!activeAgentToken) {
      return res.status(409).json({ error: 'No active agent token is available for this node.' });
    }
    const bootstrapClaim = await issueNodeBootstrapClaim({
      ...
      nodeId,
      description: 'Generated by config.yaml download.',
    });
