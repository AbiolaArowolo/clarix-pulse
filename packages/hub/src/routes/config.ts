...
import {
  consumeNodeBootstrapClaim,
  enrollNode,
  getActiveAgentToken,
  getNode,
  getPlayer,
  issueNodeBootstrapClaim,
  removeNode,
  resolveNodeAuthForAnyToken,
  resolveNodeAuthForToken,
} from '../store/registry';
...
export function createConfigRouter(io: SocketServer): Router {
  ...
  router.post('/node/bootstrap', async (req: Request, res: Response) => {
    const nodeId = asString(req.body?.nodeId ?? req.body?.node_id);
    const bootstrapClaim = asString(
      req.body?.bootstrapClaim
      ?? req.body?.bootstrap_claim
      ?? req.body?.claim,
    );
    ...
    if (!bootstrapClaim) {
      return res.status(400).json({ error: 'bootstrapClaim is required.' });
    }

    const consumed = await consumeNodeBootstrapClaim({
      nodeId,
      bootstrapClaim,
    });
    if (!consumed) {
      return res.status(401).json({ error: 'Bootstrap claim is invalid, expired, or already used.' });
    }

    const node = await getNode(consumed.tenantId, consumed.nodeId);
    if (!node) {
      return res.status(404).json({ error: 'Unknown node.' });
    }

    const agentToken = await getActiveAgentToken(consumed.nodeId, consumed.tenantId);
    if (!agentToken) {
  ...
  router.post('/remote/provision', async (req: Request, res: Response) => {
    try {
      ...
      const configYaml = serializeAgentConfigYaml(draft, {
        bootstrapClaim: bootstrapClaim.bootstrapClaim,
        bootstrapClaimExpiresAt: bootstrapClaim.expiresAt,
      });
