...
/api/config/remote/install-handoff-link
```
...
- authenticated operators can mint an expiring public `/install-handoff?token=...` page for a specific provisioned node
...
4. Generate an install handoff page from `Remote Setup` and confirm a signed-out browser can open it and access both installer and config actions.
...
3. Provision a node.
4. Optionally create the install handoff page and pass it to the field operator who will finish the Windows-side steps.
5. Import the generated `config.yaml` into the local UI or paste the secure config link.
6. Confirm heartbeats appear only in the correct tenant.
...
- VPS operations checklist: [VPS_OPERATIONS_CHECKLIST.md](/D:/monitoring/docs/VPS_OPERATIONS_CHECKLIST.md)
